...
class Posts_Maintenance extends Base {
	...
	public function add_admin_menu() {
		$hook = add_submenu_page(
			'wpmudev_plugintest_drive', // Parent slug from Google_Drive class.
					...
					$.post(ajaxurl, {
						...
						if (response.success) {
							...
							if (response.data.done) {
								$('#progress-text').text('Scan completed! Processed ' + processed + ' posts.');
								...
								$('#progress-text').text('Processed ' + processed + ' posts...');
	...
	public function handle_scan_ajax() {
		...
		foreach ( $post_types as $post_type ) {
			...
			// If fewer posts than batch size, done for this type.
	...
	public function cli_scan( $args, $assoc_args ) {
		$post_types = $assoc_args['post_types'] ?? '';
		if ( $post_types ) {
	...
	public function enqueue_admin_scripts( $hook ) {
		if ( 'google-drive-test_page_wpmudev-posts-maintenance' !== $hook ) {
			return;
		}
		// Additional JS/CSS if needed beyond inline.
	}
	...
	public function admin_body_classes( $classes = '' ) {
		...
		if ( empty( $current_screen->id ) || ! strpos( $current_screen->id, 'wpmudev-posts-maintenance' ) ) {
			return $classes;
		}
